...
进入“项目管理”，打开项目详情并选择“编辑项目信息”，在“内容识别 Tag”中填写完整标签，例如 `#MAXSUN`。内容监测会在该项目合作的预计发布日期窗口内读取对应 YouTube 频道，并只关联标题或 Description 中包含这个完整 Tag 的视频。修改后下一轮扫描立即生效；多个项目可以设置不同 Tag，避免同一达人同期合作时归属混淆。
